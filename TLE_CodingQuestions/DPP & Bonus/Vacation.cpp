...
int main() {
    ...
    vector<vector<int>> pre(n, vector<int>(m)); // Fixed syntax
    for(int i = 0; i < n; i++) {
        for(int j = 0; j < m; j++) {
            pre[i][j] = matrix[i][j];
    ...
    while(q--) {
        ...
        cin >> l1 >> r1 >> l2 >> r2;
        int sum = pre[l2][r2];
        if(l1 > 0) sum -= pre[l1-1][r2];
        if(r1 > 0) sum -= pre[l2][r1-1];
        if(l1 > 0 && r1 > 0) sum += pre[l1-1][r1-1];
        cout << sum << endl;
    }
    return 0;
}
...
signed main() {
    ...
    while(q--) {
        ...
        if(r1 > 0) sum -= pre[l2][r1-1];
        if(l1 > 0 && r1 > 0) sum += pre[l1-1][r1-1];
        if(sum == (r2-r1+1)*(l2-l1+1))cout<<1<<endl;
        else cout<<0<<endl;
        // cout << sum << endl;
    }
}
